...
class _QuizState extends State<Quiz> {
  ...
  calculateScore(index, _text) {
    var words = _text.split(" ");
    var content = quizWords.keys.elementAt(index).split(" ");
    this.wrong = (content.where((e) => !words.contains(e))).toList();
    this.correct = this.content.length - this.wrong.length.toDouble();
    score[index] = correct;
    if (index == 2) {
      // update correct score to appropriate reading level
      score.sort();
      correct = score[score.length - 1];
    }
  }
